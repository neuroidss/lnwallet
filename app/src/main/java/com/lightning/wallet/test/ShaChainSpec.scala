...
import com.lightning.wallet.ln.crypto.{ShaChain, ShaHashesWithIndex}
import com.lightning.wallet.ln.crypto.ShaChain.{largestIndex, Index}
...
class ShaChainSpec {
  ...
  def byteSeqsEqual(s1: Seq[Bytes], s2: Seq[Bytes]) =
    s1.zip(s2).forall { case (a, b) => java.util.Arrays.equals(a, b) }
