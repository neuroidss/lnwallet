...
case class Branch(parent: Option[Branch], value: Bytes, height: Int)
...
object ShaChain { me =>
  ...
  def flip(node: Branch): Bytes = flip(node.value, 63 - node.height)
  def flip(in: Bytes, index: Int): Bytes = in.updated(index / 8, in(index / 8).^(1 << index % 8).toByte)
  def deriveHash(node: Branch, right: Boolean): Bytes = if (right) Sha256Hash hash flip(node) else node.value
  def deriveChild(node: Branch, right: Boolean) = Branch(Some(node), deriveHash(node, right), node.height + 1)
  def shaChainFromSeed(hash: Bytes, idx: Long) = derive(Branch(None, hash, 0), me moves idx).value
  def derive(node: Branch, treeDirections: Index) = (node /: treeDirections)(deriveChild)
  ...
  if (index.last) hashes.updated(index, hash) else index dropRight 1 match { case index1 =>
    val check = deriveChild(node = Branch(parent = None, hash, index1.length), right = true).value
  ...
  def getHash(hashes: IndexHashMap)(index: Index) =
    hashes.keys collectFirst { case idx if index startsWith idx =>
      val startingNode = Branch(None, hashes(idx), idx.length)
